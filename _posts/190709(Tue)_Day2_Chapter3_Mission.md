# 190709(Tue)\_Day2\_Chapter3_Mission
...
def preprocess_data(filename):
    ...
    with open(filename) as file:
        ...
        for title, user_list in loaded.items():
            # processed 딕셔너리에 title을 키로, user를 값으로 저장합니다.
            processed[int(title)] = []
            
            for user in user_list:
                processed[int(title)].append(int(user))
...
def reformat_data(title_to_users):
    ...
    for title, user_list in title_to_users.items():
        # user_to_titles에 사용자 정보가 있을 경우 사용자의 영화 정보를 추가합니다. 이때 영화 정보는 리스트형으로 저장됩니다. 
        for user in user_list:
            if not(user in user_to_titles):
                user_to_titles[user] = []
