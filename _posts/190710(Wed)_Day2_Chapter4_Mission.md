# 190710(Wed)\_Day2\_Chapter4_Mission
...
def get_popular_tags(talks, n):
    ...
    for talk in talks:
        # for tag in talk['tag']:
        #     if not(tag in tag_to_views):
        #         tag_to_views[tag] = 0
            
        tag_to_views = {tag : tag_to_views[tag] + 1 if tag in tag_to_views else 1 for tag in talk['tags'] }
    ...
    top_tags = map(itemgetter(0), tag_view_pairs)
...
def completion_rate_by_duration(talks):
    ...
    '''
    durations = list(map(lambda talk: talk['duration'],talks))
    completion_rates = list(map(lambda talk: talk['comments']/talk['views'], talks))
...
def views_by_languages(talks):
    ...
    '''
    languages = list(map(lambda talk: talk['languages'], talks))
    views = list(map(lambda talk: talk['views'], talks))
...
def show_ratings(talk):
    ...
    keywords = list(map(lambda rating: rating['name'], talk['ratings']))
    counts = list(map(lambda rating: rating['count'], talk['ratings']))
